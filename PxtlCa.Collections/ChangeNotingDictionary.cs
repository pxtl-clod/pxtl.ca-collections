...
public class ChangeNotingDictionary<K, V> : VirtualDictionary<K, V>
where V : ICloneable {
    /// <summary>
    ...
    protected void NoteChange(K key, ChangeNoteType type, V? val) {
        if (NoteChanges) {
    ...
    public ChangeNotingDictionary(ChangeHandler onChanged, IEqualityComparer<K> comparer)
        : this(comparer) {
        Changed = onChanged;
    }

    public ChangeNotingDictionary(IEqualityComparer<K> comparer)
        : base(comparer) {
    ...
    public ChangeNotingDictionary(int capacity, IEqualityComparer<K> comparer)
        : base(capacity, comparer) {
        changes = new Dictionary<K, ChangeNote<V>>(comparer);
    }

    public ChangeNotingDictionary(IDictionary<K, V> originalDictionary)
        : base(originalDictionary) {
        changes = new Dictionary<K, ChangeNote<V>>();
    }

    public ChangeNotingDictionary(IDictionary<K, V> originalDictionary, IEqualityComparer<K> comparer)
    ...
    // copy the data.)
    public static ChangeNotingDictionary<K, V> Wrap(IDictionary<K, V> originalDictionary, IEqualityComparer<K> comparer) {
        return new ChangeNotingDictionary<K, V>(originalDictionary, comparer, true);
    }

    // Private constructor used to enable a dictionary either to be
    // copied, or used as the underying storage.
    protected ChangeNotingDictionary(IDictionary<K, V> originalDictionary, IEqualityComparer<K> comparer, bool wrap)
    ...
    public override void Add(KeyValuePair<K, V> item) {
        Changes[item.Key] = new ChangeNote<V>(ChangeNoteType.Add, item.Value);
        base.Add(item);
    }
}
